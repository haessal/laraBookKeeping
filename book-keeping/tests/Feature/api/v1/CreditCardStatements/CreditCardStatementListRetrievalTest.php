...
class CreditCardStatementListRetrievalTest extends TestCase
{
    ...
    public function test_credit_card_statement_of_specified_book_can_be_retrieved(): void
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/'.$this->book->book_id.'/creditcardstatements');

        $response->assertOk()
            ->assertJsonFragment([
                'id' => $this->creditCardStatement->credit_card_statement_id,
            ]);
    }
    ...
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/0/creditcardstatements');
    ...
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/'.$this->unavailableBook->book_id.'/creditcardstatements');
