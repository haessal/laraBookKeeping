...
class SlipEntryListRetrievalTest extends TestCase
{
    ...
    public function test_slip_entry_of_specified_book_can_be_retrieved(): void
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/'.$this->book->book_id.'/slipentries/?'
    ...
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/0/slipentries/?unknown=0');
    ...
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/'.$this->book->book_id.'/slipentries/?unknown=0');
    ...
    {
        $response = $this->actingAs($this->user)
            ->get('/api/v1/books/'.$this->unavailableBook->book_id.'/slipentries/?'
                .'from='.BookKeepingService::ORIGIN_DATE.'&'
                .'to='.Carbon::today()->format('Y-m-d').'&'
                .'debit='.$this->debit->account_id.'&'
                .'credit='.$this->credit->account_id.'&'
