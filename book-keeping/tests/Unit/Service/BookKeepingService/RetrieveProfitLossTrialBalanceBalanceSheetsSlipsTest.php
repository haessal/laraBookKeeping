...
class RetrieveProfitLossTrialBalanceBalanceSheetsSlipsTest extends TestCase
{
    ...
    public function test_it_retrieves_the_statement_of_the_default_book(): void
    {
        ...
        $result_expected = [BookKeepingService::STATUS_NORMAL, [
            ...
            'trial_balance' => $balanceSheet,
            'previous_balance_sheet' => $balanceSheet,
        ...
        $slipMock->shouldReceive('retrieveAmountFlows')
            ->once()
            ->with('1970-01-01', $previousDate, $bookId)
            ->andReturn($amountFlows);
        $slipMock->shouldReceive('retrieveAmountFlows')
            ->once()
            ->with('1970-01-01', $toDate, $bookId)
            ->andReturn($amountFlows);
