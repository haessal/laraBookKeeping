...
namespace Tests\Unit\Http\Controllers\api\v1\PutBooksDefaultActionApi;
...
class InvokeTest extends TestCase
{
    ...
    public function test_it_returns_internal_server_error_with_unexpected_bookkeeping_service_behavior(): void
    {
        ...
        $serviceMock->shouldReceive('validateUuid')
            ...
            ->with($bookId)
            ->andReturn(true);
        $serviceMock->shouldReceive('setBookAsDefault')
            ->once()
            ->with($bookId)
            ->andReturn([-1, null]);
        ...
        $controller = new PutBooksDefaultActionApi($serviceMock, $responderMock);
