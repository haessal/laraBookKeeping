...
namespace Tests\Unit\Http\Controllers\api\v1\GetBooksCreditCardStatementsCreditCardStatementIdActionApi;
...
class InvokeTest extends TestCase
{
    ...
    public function test_it_returns_internal_server_error_with_unexpected_bookkeeping_service_behavior(): void
    {
        ...
        $serviceMock->shouldReceive('validateUuid')
            ...
            ->andReturn(true);
        $serviceMock->shouldReceive('retrieveCreditCardStatement')
            ->once()
            ->with($creditCardStatementId, $bookId)
            ->andReturn([-1, null]);
        /** @var \App\Http\Responder\api\v1\CreditCardStatementJsonResponder|\Mockery\MockInterface $responderMock */
        $responderMock = Mockery::mock(CreditCardStatementJsonResponder::class);
        $responderMock->shouldNotReceive('response');
        /** @var \Illuminate\Http\Request|\Mockery\MockInterface $requestMock */
        $requestMock = Mockery::mock(Request::class);

        $controller = new GetBooksCreditCardStatementsCreditCardStatementIdActionApi($serviceMock, $responderMock);
