...
namespace Tests\Unit\Http\Controllers\api\v1\PatchSlipEntriesActionApi;
...
class InvokeTest extends TestCase
{
    ...
    public function test_it_returns_internal_server_error_with_unexpected_bookkeeping_service_behavior(): void
    {
        ...
        $serviceMock->shouldReceive('validateUuid')
            ...
            ->with($slipEntryId)
            ->andReturn(true);
        $serviceMock->shouldReceive('updateSlipEntry')
            ->once()
            ->with($slipEntryId, $slipEntry)
            ->andReturn([-1, null]);
        $serviceMock->shouldNotReceive('retrieveSlipEntry');
        ...
        $controller = new PatchSlipEntriesActionApi($serviceMock, $responderMock);
