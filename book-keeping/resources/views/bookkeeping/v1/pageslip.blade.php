...
<div id="accounts">
    <table>
                ...
                <table>
                    <form method="POST"
                        ...
                        <tr>
                            ...
                            <td class="in-top">
                                <input value="{{ $add['amount'] }}"
                                       size="15"
                                       name="amount"
                                       type="text" />
                            </td>
                            <td><input name="buttons[add]"
                                       value="{{ __('Add') }}"
                                       type="submit" /></td>
                        </tr>
                    </form>
        ...
        <tr>
            <td class="main">
                @empty($draftslip) {{ __('There is no item.') }}
                @else
                    <form method="POST"
                          action="{{ route('v1_slip') }}">
                        @csrf
                        <table>
                            ...
                            @foreach ($draftslip as $draftslipKey => $draftslipItem)
                                <tr class="{{ $draftslipItem['evenOdd'] }}">
                                    ...
                                    <td class="in-main"
                                        align="right">{{ $draftslipItem['amount'] }}</td>
                                    <td class="in-main">
                                        <input type="radio"
                                               name="modify_no"
                                               value="{{ $draftslipKey }}" />
                            ...
                            <tr>
                                <td class="in-main"
                                    colspan="5"
                                    align="right">{{ __('Total') }}</td>
                                <td class="in-main"
                                    align="right">{{ $totalamount }}</td>
                                <td><input name="buttons[delete]"
                                           value="{{ __('Delete') }}"
                                           type="submit" /></td>
                            </tr>
                        </table>
                        <input name="buttons[submit]"
                               value="OK"
                               type="submit" />
                    </form>
                @endempty
