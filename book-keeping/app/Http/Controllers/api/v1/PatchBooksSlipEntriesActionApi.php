...
class PatchBooksSlipEntriesActionApi extends AuthenticatedBookKeepingActionApi
{
    /**
    ...
    public function __invoke(Request $request, string $bookId, string $slipEntryId): JsonResponse
    {
        ...
        switch ($status) {
            case BookKeepingService::STATUS_NORMAL:
                [$retrievalStatus, $slips] = $this->BookKeeping->retrieveSlipEntry($slipEntryId, $bookId);
        ...
        foreach ($slipEntryContents as $contentsKey => $contentsItem) {
            ...
            switch ($contentsKey) {
                ...
                case 'credit_card_statement':
                    $trimmed['credit_card_statement'] = trim(strval($contentsItemStr));
                    break;
                default:
                    $success = false;
                    break;
